...
-- This script ensures there's a pending request to test with
...
-- 3. Create test upgrade request if no pending requests exist
DO $$
DECLARE
  test_user_id UUID;
  ...
  IF pending_count = 0 THEN
    ...
    IF test_user_id IS NOT NULL AND requested_plan_id IS NOT NULL THEN
      ...
      INSERT INTO upgrade_requests (
        ...
        requested_plan_id,
        'pending',
        'Test upgrade request for admin approval testing',
        NOW(),
        NOW()
      );
...
-- 6. Final status
SELECT 'Final status - ready for testing:' as info;
SELECT 
  COUNT(*) as total_requests,
  COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending_requests,
  COUNT(CASE WHEN status = 'approved' THEN 1 END) as approved_requests,
