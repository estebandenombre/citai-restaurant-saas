-- Fix order_items table structure and add missing columns
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE order_items ADD COLUMN dish_price DECIMAL(10,2);
...
FROM information_schema.columns 
WHERE table_name = 'order_items' 
  AND column_name IN ('id', 'order_id', 'dish_name', 'dish_price', 'quantity', 'special_instructions');

-- Test inserting a sample order item to verify structure
SELECT 'Testing order_items table with sample data:' as info;
INSERT INTO order_items (order_id, dish_name, dish_price, quantity, special_instructions)
VALUES (
