...
-- This script fixes registration, data recovery, and constraint issues
...
-- 4. Fix role constraints by updating invalid roles first
SELECT '4. Fixing role constraints:' as info;

-- Check current roles
SELECT 'Current roles in system:' as check_type;
SELECT 
    role,
    COUNT(*) as user_count
FROM users
...
UPDATE users 
SET role = 'staff'
WHERE role IN ('user', 'trial', 'starter', 'pro', 'multi');

UPDATE users 
SET role = 'staff'
WHERE role NOT IN ('owner', 'manager', 'staff');

-- Now apply the role constraint
...
-- 5. Fix plan_status constraints
SELECT '5. Fixing plan_status constraints:' as info;

-- Update invalid plan_status values
UPDATE users 
...
SELECT '6. Ensuring required columns exist:' as info;
DO $$
BEGIN
...
SELECT '7. Updating existing users:' as info;
UPDATE users SET
...
-- 8. Ensure restaurants table exists with original structure
SELECT '8. Ensuring restaurants table exists:' as info;
...
-- 9. Restore user-restaurant connections
SELECT '9. Restoring user-restaurant connections:' as info;
...
-- 10. Create safe RLS policies
SELECT '10. Creating safe RLS policies:' as info;
...
-- Policy for owner access (for users with owner role)
CREATE POLICY "owner_manage_all_users" ON users
    FOR ALL
    USING (
        EXISTS (
            SELECT 1 FROM users 
...
-- 11. Re-enable RLS
SELECT '11. Re-enabling RLS:' as info;
ALTER TABLE users ENABLE ROW LEVEL SECURITY;

-- 12. Final verification
SELECT '12. Final verification:' as info;
SELECT 
...
-- 13. Show final user-restaurant mapping
SELECT '13. Final user-restaurant mapping:' as info;
SELECT 
    u.email,
    u.first_name,
    u.last_name,
...
-- 14. Success message
SELECT '14. Complete fix applied successfully:' as info;
SELECT '✅ All constraint violations resolved' as status;
